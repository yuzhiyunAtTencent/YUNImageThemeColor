...
- (void)configureLumaAndSaturationWithMode:(PaletteTargetMode)mode{
    // light指的是亮度大，对应的反面是dark(亮对应暗)
    // vibrant指的是鲜艳，饱和度高，对应的反面是muted,弱饱和度的
...
+ (NSString*)getTargetModeKey:(PaletteTargetMode)mode{
    ...
    switch (mode) {
        case LIGHT_VIBRANT_PALETTE:
            key = PALETTE_LIGHT_VIBRANT;
            break;
        case VIBRANT_PALETTE:
            key = PALETTE_VIBRANT;
            break;
        case DARK_VIBRANT_PALETTE:
            key = PALETTE_DARK_VIBRANT;
            break;
        case LIGHT_MUTED_PALETTE:
            key = PALETTE_LIGHT_MUTED;
            break;
        case MUTED_PALETTE:
            key = PALETTE_MUTED;
            break;
        case DARK_MUTED_PALETTE:
            key = PALETTE_DARK_MUTED;
            break;
        default:
            break;
    }
